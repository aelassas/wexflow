        ...
        public override IEnumerable<Core.Db.User> GetAdministrators(string keyword, UserOrderBy uo)
        {
            lock (Padlock)
            {
                ...
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + User.ColumnNameId + ", "
        ...
        public override DateTime GetEntryStatusDateMax()
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + Entry.ColumnNameStatusDate
                        + " FROM " + Core.Db.Entry.DocumentName
        ...
        public override DateTime GetHistoryEntryStatusDateMax()
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + HistoryEntry.ColumnNameStatusDate
                        + " FROM " + Core.Db.HistoryEntry.DocumentName
        ...
        public override string GetPassword(string username)
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + User.ColumnNamePassword
                         + " FROM " + Core.Db.User.DocumentName
        ...
        public override Core.Db.StatusCount GetStatusCount()
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + StatusCount.ColumnNameId + ", "
                        + StatusCount.ColumnNamePendingCount + ", "
                        + StatusCount.ColumnNameRunningCount + ", "
                        + StatusCount.ColumnNameDoneCount + ", "
                        + StatusCount.ColumnNameFailedCount + ", "
        ...
        public override IEnumerable<string> GetUserWorkflows(string userId)
        {
            lock (Padlock)
            {
                ...
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + UserWorkflow.ColumnNameId + ", "
        ...
        public override Core.Db.Workflow GetWorkflow(string id)
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + Workflow.ColumnNameId + ", "
        ...
        public override string GetEntryLogs(string entryId)
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + Entry.ColumnNameLogs
                         + " FROM " + Core.Db.Entry.DocumentName
                         + " WHERE "
        ...
        public override string GetHistoryEntryLogs(string entryId)
        {
            lock (Padlock)
            {
                using (var conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (var command = new NpgsqlCommand("SELECT " + HistoryEntry.ColumnNameLogs
                        + " FROM " + Core.Db.HistoryEntry.DocumentName
                        + " WHERE "
