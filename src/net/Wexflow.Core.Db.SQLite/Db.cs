    ...
    public sealed class Db : Core.Db.Db
    {
        ...
        public override void Init()
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.StatusCount.DocumentName + "("
            ...
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand("SELECT COUNT(*) FROM " + Core.Db.User.DocumentName + ";", conn))
                {
        ...
        public override bool CheckUserWorkflow(string userId, string workflowId)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT COUNT(*) FROM " + Core.Db.UserWorkflow.DocumentName
        ...
        public override void ClearEntries()
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Entry.DocumentName + ";", conn))
                    {
                        command.ExecuteNonQuery();
                    }
                }
        ...
        public override void ClearStatusCount()
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.StatusCount.DocumentName + ";", conn))
                    {
                        command.ExecuteNonQuery();
                    }
                }
        ...
        public override void DeleteUser(string username, string password)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.User.DocumentName
        ...
        public override void DeleteUserWorkflowRelationsByUserId(string userId)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.UserWorkflow.DocumentName
        ...
        public override void DeleteWorkflow(string id)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Workflow.DocumentName
        ...
        public override IEnumerable<Core.Db.User> GetAdministrators(string keyword, UserOrderBy uo)
        {
            lock (padlock)
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + User.ColumnName_Id + ", "
        ...
        public override IEnumerable<Core.Db.Entry> GetEntries(string keyword, DateTime from, DateTime to, int page, int entriesCount, EntryOrderBy eo)
        {
            lock (padlock)
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand(sqlBuilder.ToString(), conn))
                    {
        ...
        public override long GetEntriesCount(string keyword, DateTime from, DateTime to)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT COUNT(*)"
                        + " FROM " + Core.Db.Entry.DocumentName
                        + " WHERE " + "(LOWER(" + Entry.ColumnName_Name + ") LIKE '%" + (keyword ?? "").Replace("'", "''").ToLower() + "%'"
                        + " OR " + "LOWER(" + Entry.ColumnName_Description + ") LIKE '%" + (keyword ?? "").Replace("'", "''").ToLower() + "%')"
        ...
        public override DateTime GetEntryStatusDateMax()
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + Entry.ColumnName_StatusDate
                        + " FROM " + Core.Db.Entry.DocumentName
                        + " ORDER BY " + Entry.ColumnName_StatusDate + " DESC LIMIT 1;", conn))
                    {
                        ...
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                DateTime statusDate = DateTime.Parse((string)reader[Entry.ColumnName_StatusDate]);
        ...
        public override DateTime GetHistoryEntryStatusDateMax()
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + HistoryEntry.ColumnName_StatusDate
                        + " FROM " + Core.Db.HistoryEntry.DocumentName
                        + " ORDER BY " + HistoryEntry.ColumnName_StatusDate + " DESC LIMIT 1;", conn))
                    {
                        ...
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            ...
                            if (reader.Read())
                            {
                                DateTime statusDate = DateTime.Parse((string)reader[HistoryEntry.ColumnName_StatusDate]);
        ...
        public override string GetPassword(string username)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + User.ColumnName_Password
                        + " FROM " + Core.Db.User.DocumentName
        ...
        public override Core.Db.StatusCount GetStatusCount()
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + StatusCount.ColumnName_Id + ", "
                        + StatusCount.ColumnName_PendingCount + ", "
                        + StatusCount.ColumnName_RunningCount + ", "
                        + StatusCount.ColumnName_DoneCount + ", "
                        + StatusCount.ColumnName_FailedCount + ", "
        ...
        public override IEnumerable<string> GetUserWorkflows(string userId)
        {
            lock (padlock)
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + UserWorkflow.ColumnName_Id + ", "
                        ...
                        + ";", conn))
                    {
                        ...
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                long workflowId = (long)reader[UserWorkflow.ColumnName_WorkflowId];
        ...
        public override Core.Db.Workflow GetWorkflow(string id)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + Workflow.ColumnName_Id + ", "
        ...
        private static void IncrementStatusCountColumn(string statusCountColumnName)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.StatusCount.DocumentName + " SET " + statusCountColumnName + " = " + statusCountColumnName + " + 1;", conn))
                    {
                        command.ExecuteNonQuery();
                    }
                }
        ...
        public override void InsertEntry(Core.Db.Entry entry)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Entry.DocumentName + "("
                        + Entry.ColumnName_Name + ", "
                        + Entry.ColumnName_Description + ", "
                        + Entry.ColumnName_LaunchType + ", "
                        + Entry.ColumnName_StatusDate + ", "
        ...
        public override void InsertHistoryEntry(Core.Db.HistoryEntry entry)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.HistoryEntry.DocumentName + "("
                        + HistoryEntry.ColumnName_Name + ", "
                        + HistoryEntry.ColumnName_Description + ", "
                        + HistoryEntry.ColumnName_LaunchType + ", "
                        + HistoryEntry.ColumnName_StatusDate + ", "
        ...
        public override void InsertUser(Core.Db.User user)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.User.DocumentName + "("
                        + User.ColumnName_Username + ", "
                        + User.ColumnName_Password + ", "
                        + User.ColumnName_UserProfile + ", "
                        + User.ColumnName_Email + ", "
        ...
        public override void InsertUserWorkflowRelation(Core.Db.UserWorkflow userWorkflow)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.UserWorkflow.DocumentName + "("
                        + UserWorkflow.ColumnName_UserId + ", "
        ...
        public override string InsertWorkflow(Core.Db.Workflow workflow)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Workflow.DocumentName + "("
        ...
        public override void UpdateEntry(string id, Core.Db.Entry entry)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Entry.DocumentName + " SET "
        ...
        public override void UpdatePassword(string username, string password)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.User.DocumentName + " SET "
        ...
        public override void UpdateWorkflow(string dbId, Core.Db.Workflow workflow)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Workflow.DocumentName + " SET "
        ...
        public override string GetEntryLogs(string entryId)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + Entry.ColumnName_Logs
                        + " FROM " + Core.Db.Entry.DocumentName
                        + " WHERE "
        ...
        public override string GetHistoryEntryLogs(string entryId)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT " + HistoryEntry.ColumnName_Logs
                        + " FROM " + Core.Db.HistoryEntry.DocumentName
                        + " WHERE "
        ...
        public override string InsertRecord(Core.Db.Record record)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Record.DocumentName + "("
        ...
        public override void UpdateRecord(string recordId, Core.Db.Record record)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Record.DocumentName + " SET "
            ...
            lock (padlock)
            {
                if (recordIds.Length > 0)
                {
                    using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                    {
                        ...
                        using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Record.DocumentName
        ...
        public override string InsertVersion(Core.Db.Version version)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Version.DocumentName + "("
        ...
        public override void UpdateVersion(string versionId, Core.Db.Version version)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Version.DocumentName + " SET "
            ...
            lock (padlock)
            {
                if (versionIds.Length > 0)
                {
                    using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                    {
                        ...
                        using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Version.DocumentName
        ...
        public override string InsertNotification(Core.Db.Notification notification)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Notification.DocumentName + "("
        ...
        public override void MarkNotificationsAsRead(string[] notificationIds)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Notification.DocumentName
            ...
            lock (padlock)
            {
                if (notificationIds.Length > 0)
                {
                    using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                    {
                        ...
                        using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Notification.DocumentName
                            + " WHERE " + Notification.ColumnName_Id + " IN " + builder.ToString() + ";", conn))
                        {
                            command.ExecuteNonQuery();
                        }
        ...
        public override string InsertApprover(Core.Db.Approver approver)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("INSERT INTO " + Core.Db.Approver.DocumentName + "("
        ...
        public override void UpdateApprover(string approverId, Core.Db.Approver approver)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("UPDATE " + Core.Db.Approver.DocumentName + " SET "
        ...
        public override void DeleteApproversByRecordId(string recordId)
        {
            lock (padlock)
            {
                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("DELETE FROM " + Core.Db.Approver.DocumentName
