                    ...
                    if (recordIds != null)
                    {
                        foreach (var recordId in recordIds)
                        {
                            try
                            {
                                ...
                                var workflowName =
                                    $"Workflow_ApproveRecord_{SecurityElement.Escape(Approver)}_{SecurityElement.Escape(record.Name)}";

                                var xml =
                                    $"<Workflow xmlns='urn:wexflow-schema' id='{workflowId}' name='{workflowName}' description='{workflowName}'>\r\n"
                                    + "	 <Settings>\r\n"
                                ...
                                if (workflowDbId != "-1")
                                {
                                    ...
                                    {
                                        var filePath = Path.Combine(Workflow.WexflowEngine.WorkflowsFolder,
                                    ...
                                    Info(
                                        $"Approval Workflow of the record {recordId} - {record.Name} created and started successfully.");
                                ...
                                {
                                    Error(
                                        $"An error occured while creating the approval workflow of the record {recordId} - {record.Name}.");
                                    success = false;
                                }
                            }
                            ...
                            {
                                ErrorFormat("An error occured while creating the approval workflow for the record {0}.",
                                    e, recordId);
                                success = false;
                            }
                            finally
